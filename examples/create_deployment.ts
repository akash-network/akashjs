...
import { QueryBidsRequest, QueryLeasesRequest, QueryClientImpl as QueryMarketClient } from "../build/protobuf/akash/market/v1beta4/query";
import { QueryProviderRequest, QueryClientImpl as QueryProviderClient } from "../build/protobuf/akash/provider/v1beta3/query";
...
async function createLease(deployment: Deployment, wallet: DirectSecp256k1HdWallet, client: SigningStargateClient): Promise<Lease> {
    const { id: { dseq, owner } } = deployment;
...
async function fetchLease(deployment: Deployment, wallet: DirectSecp256k1HdWallet): Promise<Lease> {
    const rpc = await getRpc(rpcEndpoint);
    const client = new QueryMarketClient(rpc);
    const tx = await client.Leases(QueryLeasesRequest.fromPartial({
        filters: {
    ...
    const lease = tx.leases[0]?.lease;

    if (lease === undefined) {
        throw new Error(`Could not find lease for deployment ${deployment.id.dseq}`);
    }

    return {
        id: {
            owner: lease.leaseId?.owner || "",
            dseq: lease.leaseId?.dseq.toNumber() || 0,
    ...
    while (Date.now() - startTime < timeout) {
        ...
        if (status) {
            for (const [name, service] of Object.entries(status.services)) {
                if ((service as any).uris) {
                    console.log(`Service ${name} is available at:`, (service as any).uris[0]);
                    return;
                }
            }
        }
