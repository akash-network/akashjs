...
tap.test("Deployments: query deployment list with owner filter", async (t) => {
  ...
  const client = new QueryClientImpl(
    await getRpc("https://rpc.akash.forbole.com:443")
  );
  ...
  testSnap(t)("Deployments query matches expected result")(
    QueryDeploymentsResponse.toJSON(res)
  );
});
...
tap.test("Deployments: query deployment with owner and dseq", async (t) => {
  ...
  testSnap(t)("Deployments query matches expected result")(
    QueryDeploymentResponse.toJSON(res)
  );
});
